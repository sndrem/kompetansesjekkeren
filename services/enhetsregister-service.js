...
const enhetsregisterService = {
    hentEnhetsdata: async function (orgnr) {
        const hovedenhet = rp(
            `${ENHETSREGISTERET_HOST_AND_PORT_HOVEDENHETER
            }?organisasjonsnummer=${orgnr}`
        );

        const underenhet = rp(
            `${ENHETSREGISTERET_HOST_AND_PORT_UNDERENHETER
            }?organisasjonsnummer=${orgnr}`
        );
        ...
        try {
            ...
            if (parsed["_embedded"] && parsed["_embedded"]["enheter"]) {
                ...
                if (parsedUnderenhet["_embedded"] && parsedUnderenhet["_embedded"]["underenheter"]) {
